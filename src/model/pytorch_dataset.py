    ...
    def _identify_categorical_columns(self) -> List[str]:
        ...
        for col in self.df.columns:
            ...
            if (col in known_categorical or 
                (self.df[col].dtype in ['int64', 'int8'] and 
                 col.startswith(('ProductCD', 'card', 'email', 'Device', 'id_')))):
    ...
    def _identify_numerical_columns(self) -> List[str]:
        ...
        for col in self.df.columns:
            if (col != self.target_column and 
                col != 'TransactionID' and 
                col not in self.categorical_columns and
                self.df[col].dtype in ['int64', 'int8', 'float64', 'float32']):
    ...
    def _prepare_tensors(self) -> Tuple[Dict[str, torch.Tensor], torch.Tensor]:
        ...
        if self.target_column in self.df.columns:
            targets = torch.tensor(
                self.df[self.target_column].values.astype(np.float32), 
                dtype=torch.float32, 
                device=self.device
            )
        else:
...
def create_fraud_dataloader(dataset: PyTorch_Dataset,
                           batch_size: int = 256,
                           shuffle: bool = True,
