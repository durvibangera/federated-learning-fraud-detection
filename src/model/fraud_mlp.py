...
class FraudMLP(nn.Module):
    ...
    def __init__(self,
        ...
        logger.info(f"FraudMLP initialized: {self.total_input_dim} input features, "
    ...
    def forward(self, features: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        if embeddings_list:
            ...
            categorical_embeddings = torch.empty(features.get('numerical', features.get('categorical')).size(0), 0, 
        ...
        if 'numerical' in features:
            ...
            numerical_features = torch.empty(categorical_embeddings.size(0), 0, 
...
def create_fraud_model(categorical_embedding_dims: Dict[str, Tuple[int, int]],
                      numerical_input_dim: int,
                      class_weights: Optional[torch.Tensor] = None,
                      hidden_dims: List[int] = [256, 128, 64],
                      dropout_rate: float = 0.3,
                      device: str = 'cpu') -> Tuple[FraudMLP, FraudLoss, torch.optim.Optimizer]:
    """
...
def validate_model_architecture(model: FraudMLP, 
                               sample_features: Dict[str, torch.Tensor]) -> Dict[str, Any]:
    """
