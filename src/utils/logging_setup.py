...
def log_privacy_budget(epsilon_spent: float, delta: float, round_number: int) -> None:
    ...
    ).info("Privacy budget updated")


def log_federated_round(round_num: int, participating_clients: list, 
    ...
    ).info(f"Federated learning round {round_num} completed")
...
def log_error_with_context(error: Exception, context: Dict[str, Any]) -> None:
    ...
    ).error(f"Error occurred: {str(error)}")