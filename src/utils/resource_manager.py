...
class Resource_Manager:
    ...
    def adapt_batch_size(
        ...
        if force_reduction or (under_pressure and severity == 'critical'):
            ...
            new_batch_size = min(
                ...
                int(current_batch_size * self.config.batch_size_increase_factor)
            )
    ...
    def _check_quarantine_size(self) -> None:
        ...
        total_size = sum(
            f.stat().st_size for f in quarantine_path.rglob('*') if f.is_file()
        )
    ...
    def cleanup_old_files(
        ...
        for directory in directories:
            ...
            if removed_count > 0:
                ...
                logger.info(
                    f"Cleaned {directory}: removed {removed_count} files, "
    ...
    def handle_out_of_memory(self) -> bool:
        ...
        if self.current_batch_size:
            self.current_batch_size = self.adapt_batch_size(
    ...
    def get_recommended_batch_size(
        ...
        recommended_batch_size = max(
            self.config.min_batch_size,
            min(self.config.max_batch_size, estimated_batch_size)
        )
