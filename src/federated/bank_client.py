...
class Bank_Client(fl.client.NumPyClient):
    ...
    def __init__(
        ...
        self.training_history = {"rounds": [], "train_loss": [], "val_loss": [], "val_auprc": [], "val_auroc": []}
    ...
    def fit(self, parameters: List[np.ndarray], config: Dict[str, str]) -> Tuple[List[np.ndarray], int, Dict]:
        ...
        for epoch in range(self.local_epochs):
            ...
            if (epoch + 1) % 2 == 0:
                logger.debug(
                    f"{self.bank_id}: Epoch {epoch+1}/{self.local_epochs}, "
        ...
        metrics = {"train_loss": avg_loss, "num_examples": num_examples, "local_epochs": self.local_epochs}
        ...
        logger.info(f"{self.bank_id}: Completed local training - " f"Loss={avg_loss:.4f}, Samples={num_examples}")
    ...
    def evaluate(self, parameters: List[np.ndarray], config: Dict[str, str]) -> Tuple[float, int, Dict]:
        ...
        metrics = {"loss": avg_loss, "auprc": auprc, "auroc": auroc, "num_examples": num_examples}

        logger.info(
            f"{self.bank_id}: Evaluation complete - " f"Loss={avg_loss:.4f}, AUPRC={auprc:.4f}, AUROC={auroc:.4f}"
        )
