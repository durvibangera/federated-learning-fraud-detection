...
class Data_Preprocessor:
    ...
    def merge_datasets(self, 
        ...
        logger.info(f"Merged datasets: {merge_stats['merged_records']} records, "
                   f"{merge_stats['identity_match_rate']:.2f}% identity match rate")
        
        return merged_df
    
    def partition_by_product_cd(self, 
                               df: pd.DataFrame, 
                               num_banks: int = 3) -> Dict[str, pd.DataFrame]:
        """
        ...
        for bank_id in [f"bank_{i+1}" for i in range(num_banks)]:
            ...
            logger.info(f"{bank_id}: {len(bank_data)} records, "
    ...
    def encode_categorical_features(self, 
                                  ...
                                  test_df: Optional[pd.DataFrame] = None) -> Tuple[pd.DataFrame, ...]:
        """
        ...
        for col in train_df.columns:
            if (train_df[col].dtype == 'category' or 
                train_df[col].dtype == 'object' or 
                col in ['ProductCD', 'card4', 'card6', 'P_emaildomain', 'R_emaildomain', 
    ...
    def preprocess_full_pipeline(self, 
                                ...
                                num_banks: int = 3) -> Dict[str, Dict[str, pd.DataFrame]]:
        """
        ...
        for bank_id, bank_data in bank_partitions.items():
            ...
            encoded_train, encoded_val, encoded_test = self.encode_categorical_features(
            ...
            logger.info(f"{bank_id} preprocessing complete: "
