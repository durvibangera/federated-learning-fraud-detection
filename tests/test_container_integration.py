...
class TestContainerOrchestration:
    ...
    def test_docker_compose_structure(self, docker_compose_config):
        ...
        assert any('aggregation' in name.lower() or 'server' in name.lower() 
                  for name in services.keys())
        
        # Should have bank clients
        bank_services = [name for name in services.keys() 
    ...
    def test_volume_mounts(self, docker_compose_config):
        ...
        services_with_volumes = [
            ...
            if 'volumes' in config and len(config['volumes']) > 0
        ]
    ...
    def test_port_mappings(self, docker_compose_config):
        ...
        for name, config in services.items():
            if 'aggregation' in name.lower() or 'server' in name.lower():
                ...
                assert 'ports' in config or 'expose' in config, \
                    "Aggregation server should expose ports"
        ...
        for dockerfile_path in dockerfiles:
            if dockerfile_path.exists():
                ...
                assert 'requirements' in content.lower(), \
                    f"{dockerfile_path.name} should copy requirements"
